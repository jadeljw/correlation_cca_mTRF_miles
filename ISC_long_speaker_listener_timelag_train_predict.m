...
%         train_corr_unattend_r = zeros(60,15);
        ...
        for story = 1 : 15
            ...
            train_corr_attend_listener_w = zeros(length(listener_chn),1);
            train_corr_unattend_listener_w = zeros(length(listener_chn),1);
            
            % cca
%             [train_corr_attend_listener_w,train_corr_attend_speaker_w,train_corr_attend_r] = canoncorr(story_train_listener_EEG',story_train_speaker_Attend');
%             [train_corr_unattend_listener_w,train_corr_unattend_speaker_w,train_corr_unattend_r] = canoncorr(story_train_listener_EEG',story_train_speaker_unAttend');
            for chn = 1 : length(listener_chn)
                train_corr_attend_listener_w(chn) = corr(story_train_listener_EEG(chn,:)',story_train_speaker_Attend(chn,:)');
                train_corr_unattend_listener_w(chn) = corr(story_train_listener_EEG(chn,:)',story_train_speaker_unAttend(chn,:)');
            end
            
            train_corr_attend_speaker_w = train_corr_attend_listener_w;
            train_corr_unattend_speaker_w = train_corr_unattend_listener_w;
            
            % record into one matrix
            train_corr_attend_listener_w_mean(:,story) = train_corr_attend_listener_w;
            train_corr_attend_speaker_w_mean(:,story) = train_corr_attend_speaker_w;
%             train_corr_attend_r(:,story) = train_corr_attend_r;
            train_corr_unattend_listener_w_mean(:,story) = train_corr_unattend_listener_w;
            train_corr_unattend_speaker_w_mean(:,story) = train_corr_unattend_speaker_w;
%             train_corr_unattend_r(:,story) = train_corr_unattend_r;
