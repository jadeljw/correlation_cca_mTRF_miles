...
recon_AttendDecoder_attend_total = zeros(1,length(lambda));
recon_AttendDecoder_unattend_total = zeros(1,length(lambda));
recon_UnattendDecoder_attend_total = zeros(1,length(lambda));
recon_UnattendDecoder_unattend_total = zeros(1,length(lambda));
decoding_acc_attended = zeros(1,length(lambda));
decoding_acc_unattended = zeros(1,length(lambda));
Decoding_acc_attend_ttest_result = zeros(1,length(lambda));
Decoding_acc_unattend_ttest_result = zeros(1,length(lambda));


bandName = ' broadband 0.1-40Hz lambda';
...
figure; plot(log2(lambda),recon_AttendDecoder_attend_total,'r');
hold on; plot(log2(lambda),recon_AttendDecoder_unattend_total,'b');
% xlabel('Times(ms)'); 
xlabel('log2-lambda-value');
ylabel('r-value')
...
figure; plot(log2(lambda),recon_UnattendDecoder_attend_total,'r');
hold on; plot(log2(lambda),recon_UnattendDecoder_unattend_total,'b');
% xlabel('Times(ms)'); 
xlabel('log2-lambda-value');
ylabel('r-value')
...
figure; plot(log2(lambda),decoding_acc_attended*100,'r');
a = log2(lambda);
hold on;plot(a(Decoding_acc_attend_ttest_result>0),decoding_acc_attended(Decoding_acc_attend_ttest_result>0)*100,'r*');
hold on; plot(log2(lambda),decoding_acc_unattended*100,'b');
hold on;plot(a(Decoding_acc_unattend_ttest_result>0),decoding_acc_unattended(Decoding_acc_unattend_ttest_result>0)*100,'b*');
% xlabel('Times(ms)'); 
xlabel('log2-lambda-value');
