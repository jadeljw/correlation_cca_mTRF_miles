...
for j = 1 : length(lambda)
    ...
    band_name = strcat(' broadband 0.5-40Hz lambda',num2str(lambda(j)));
    %% mTRF
    ...
    for listener = 1 : 12
        ...
        for story = 1 : 15
            ...
            % mTRF
            [w_train_mTRF_attend,t_train_mTRF_attend,con_train_mTRF_attend] = mTRFtrain(zscore(story_train_audio_Attend'),zscore(story_train_EEG'),Fs,1,start_time,end_time,lambda(j));
            [w_train_mTRF_unattend,t_train_mTRF_unattend,con_train_mTRF_unattend] = mTRFtrain(zscore(story_train_audio_unAttend'),zscore(story_train_EEG'),Fs,1,start_time,end_time,lambda(j));
    ...
    saveName1 = strcat('Reconstruction Accuracy using mTRF method for attend decoder -200~500 ms',band_name,'.jpg');
    ...
    xlabel('Subject No.'); ylabel('r value')
    saveName2 = strcat('Reconstruction Accuracy using mTRF method for unattend decoder -200~500 ms',band_name,'.jpg');
    ...
    saveName3 = strcat('Decoding accuracy using mTRF method for attend and unattend decoder -200~500 ms',band_name,'.jpg');
    title(saveName3(1:end-4))
    legend('Attend Decoder','Unattend Decoder')
    saveas(gcf,saveName3);
    close
    
    saveName = strcat('mTRF_sound_EEG_result -200~500 ms',band_name,'.mat');
